...
def create_grouped_defection_frame(
    ...
    # Cell format: tuple[yes, no, abstain], tuple[defection_yes, defection_no, defection_abstain], cohesion
    for g in group:
        print(f"\tCalculating defection matrix for '{g}'...")
        p = grouped.get_group(g)
        if len(p) <= 1:
            for vote in votes:
                frame.at[vote, g] = ((NaN, NaN, NaN), (NaN, NaN, NaN), NaN)
            continue

        for vote in votes:
            if str(vote) not in p.columns:
                frame.at[vote, g] = ((0, 0, 0), (NaN, NaN, NaN), NaN)
                continue
